...
async function updateSelfProfile(userId, updates) {
    // Sanitize
    const clean = {};
    if (updates.user_name !== undefined)
        clean.user_name = String(updates.user_name).trim();
    if (updates.email !== undefined)
        clean.email = updates.email?.toLowerCase() ?? null;
    if (updates.template_version !== undefined)
        clean.template_version = updates.template_version;
