...
exports.createDeliveriesFastifyPlugin = void 0;
...
const createDeliveriesFastifyPlugin = (_config) => {
    const plugin = (app, _opts, done) => {
        ...
        const requireRoleAndDomain = async (req, reply) => {
            ...
            if (reply.sent)
                return;
            if (!(0, roleResolver_1.hasDomain)(role, 'deliveries')) {
        ...
        const requireCap = (capKey) => async (req, reply) => {
            const role = (req.context?.role || req.params?.role || '').toLowerCase();
            const caps = (0, roleResolver_1.getDomainCapabilities)(role, 'deliveries') || {};
        ...
        const statusEnum = zod_1.z.enum(['pending', 'in_transit', 'delivered', 'cancelled']);
        const typeEnum = zod_1.z.enum(['inbound', 'outbound']);
        f.get('/', { preHandler: [requireRoleAndDomain, requireCap('view')], schema: { querystring: zod_1.z.object({ status: statusEnum.optional(), type: typeEnum.optional(), page: zod_1.z.coerce.number().int().min(1).optional(), limit: zod_1.z.coerce.number().int().min(1).max(200).optional() }) } }, async (req, reply) => {
        ...
        f.patch('/:shipmentId/status', { preHandler: [requireRoleAndDomain, requireCap('update')], schema: { params: zod_1.z.object({ shipmentId: zod_1.z.string().min(1) }), body: zod_1.z.object({ status: statusEnum }) } }, async (req, reply) => {
            const warehouseId = req.user.userId;
...
exports.createDeliveriesFastifyPlugin = createDeliveriesFastifyPlugin;
