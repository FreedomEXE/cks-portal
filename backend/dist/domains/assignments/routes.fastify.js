...
exports.createAssignmentsFastifyPlugin = void 0;
...
const createAssignmentsFastifyPlugin = (_config) => {
    const plugin = (app, _opts, done) => {
        ...
        const requireRoleAndDomain = async (req, reply) => {
            ...
            if (reply.sent)
                return;
            if (!(0, roleResolver_1.hasDomain)(role, 'assignments')) {
        ...
        const requireCap = (capKey) => async (req, reply) => {
            const role = (req.context?.role || req.params?.role || '').toLowerCase();
            const caps = (0, roleResolver_1.getDomainCapabilities)(role, 'assignments') || {};
        ...
        const statusEnum = zod_1.z.enum(['pending', 'assigned', 'in_progress', 'completed', 'cancelled']);
        const priorityEnum = zod_1.z.enum(['low', 'medium', 'high', 'urgent']);
        f.get('/', { preHandler: [requireRoleAndDomain, requireCap('view')], schema: { querystring: zod_1.z.object({ status: statusEnum.optional(), assignee_id: zod_1.z.string().optional(), page: zod_1.z.coerce.number().int().min(1).optional(), limit: zod_1.z.coerce.number().int().min(1).max(200).optional() }) } }, async (req, reply) => {
            const items = await svc.list(req.query);
            const role = (req.context?.role || req.params?.role || '').toLowerCase();
            return reply.code(200).send({ success: true, data: items, meta: { role } });
        });
        f.post('/', { preHandler: [requireRoleAndDomain, requireCap('create')], schema: { body: zod_1.z.object({ type: zod_1.z.string().default('task'), subject: zod_1.z.string().min(3).max(200), assignee_id: zod_1.z.string().min(3), priority: priorityEnum.default('medium') }) } }, async (req, reply) => {
            const { type, subject, assignee_id, priority } = req.body;
            const created = await svc.create(type, subject, assignee_id, priority);
            return reply.code(201).send({ success: true, data: created });
        });
        f.patch('/:assignmentId/status', { preHandler: [requireRoleAndDomain, requireCap('update')], schema: { params: zod_1.z.object({ assignmentId: zod_1.z.coerce.number().int().positive() }), body: zod_1.z.object({ status: statusEnum }) } }, async (req, reply) => {
...
exports.createAssignmentsFastifyPlugin = createAssignmentsFastifyPlugin;
