...
router.get('/:id', (0, requireCaps_1.requireCaps)('orders:view'), async (req, res) => {
    try {
        const managerId = req.user?.userId;
...
router.post('/', (0, requireCaps_1.requireCaps)('orders:create'), async (req, res) => {
    try {
        const managerId = req.user?.userId;
        const { customer_id, center_id, contractor_id, service_id, title, description, priority = 'medium', scheduled_date, estimated_hours, total_amount } = req.body;
        // Basic validation
...
router.patch('/:id', (0, requireCaps_1.requireCaps)('orders:edit'), async (req, res) => {
    try {
        const managerId = req.user?.userId;
        const orderId = req.params.id;
        const updates = req.body;
        // Check if order exists and user has access
        const existingOrder = await pool_1.default.query(`
      ...
      `, [orderId, managerId]);
        ...
        RETURNING *
      `;
        const result = await pool_1.default.query(updateQuery, values);
...
router.delete('/:id', (0, requireCaps_1.requireCaps)('orders:delete'), async (req, res) => {
    try {
        const managerId = req.user?.userId;
        const orderId = req.params.id;
        // Check if order exists and user has access
