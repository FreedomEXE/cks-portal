...
exports.getEntityCounts = getEntityCounts;
...
async function getEntityCounts(managerId) {
    const sql = `
  ...
  `;
    const result = await (0, connection_1.query)(sql, [managerId]);
    return result[0] || { contractors: 0, customers: 0, centers: 0, crew: 0 };
}
// Get recent activity for dashboard
async function getRecentActivity(managerId, limit = 10) {
    const sql = `
    SELECT 
      al.activity_id,
      al.action,
    ...
    LIMIT $2
  `;
    return await (0, connection_1.query)(sql, [managerId, limit]);
}
// Get order statistics for dashboard
async function getOrderStats(managerId) {
    const sql = `
...
async function getPerformanceMetrics(managerId) {
    const sql = `
