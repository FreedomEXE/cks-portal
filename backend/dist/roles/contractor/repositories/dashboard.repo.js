...
async function getEntityCounts(contractorId) {
    const sql = `
...
async function getRecentActivity(contractorId, limit = 10) {
    const sql = `
    SELECT 
      al.activity_id,
      al.action,
...
async function getJobStats(contractorId) {
    const sql = `
    WITH job_counts AS (
      SELECT 
        COUNT(*) as total,
...
async function getPerformanceMetrics(contractorId) {
    const sql = `
