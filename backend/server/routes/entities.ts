 ...
 *
 * Descriptio:
 *   Read-only entity profile endpoints (crew/contractors/customers/centers).
 * Functionality:
 *   Fetches single records by ID with minimal field projection and standardized success/error responses.
 * Importance:
 *   Powers frontend detail views; foundational for admin lookup UX.
 * Conections:
 *   Depends on db/pool for queries and utils/http for response helpers; mounted in src/index.ts.
 * Notes:
 *   Consolidated list endpoints live in src/index.ts smartList until refactored here.
 */
/*───────────────────────────────────────────────
