...
export class AdminUserService {
  ...
  async validateUserData(userData: any) {
    ...
    const usernameExists = await this.pool.query(
      'SELECT user_id FROM system_users WHERE user_name = $1 AND archived = false',
      [userData.user_name]
    );
    ...
    const emailExists = await this.pool.query(
      'SELECT user_id FROM system_users WHERE email = $1 AND archived = false',
      [userData.email]
    );
    ...
    if (userData.org_id) {
      const orgExists = await this.pool.query(
        'SELECT org_id FROM organizations WHERE org_id = $1 AND archived = false',
        [userData.org_id]
      );
    ...
    if (userData.manager_id) {
      const managerExists = await this.pool.query(
