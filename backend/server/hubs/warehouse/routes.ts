...
router.get('/profile', async (req: Request, res: Response) => {
  try {
    ...
    const r = await pool.query(
      `SELECT warehouse_id, warehouse_name, address, manager_id, warehouse_type, phone, email, date_acquired, capacity, current_utilization, status
    ...
    // Load manager details if present (best-effort)
    let manager: any = null;
    if (row.manager_id) {
      const mr = await pool.query(`SELECT manager_id, manager_name, email, phone FROM managers WHERE manager_id=$1`, [row.manager_id]);
      if (mr.rows.length) manager = mr.rows[0];
    }
...
router.get('/inventory', async (req: Request, res: Response) => {
  try {
    ...
    const totalsQuery = `
      ...
      WHERE warehouse_id = $1
    `;
    const totalsResult = await pool.query(totalsQuery, [warehouseId]);
...
router.get('/shipments', async (req: Request, res: Response) => {
  try {
    ...
    `;
    const statusResult = await pool.query(statusQuery, [warehouseId]);
...
router.post('/inventory/adjust', requirePermission('WAREHOUSE_ADJUST'), async (req: Request, res: Response) => {
  try {
    ...
    const updateQuery = `
      ...
      RETURNING *
    `;
    const result = await pool.query(updateQuery, [quantity_change, item_id, warehouseId]);
    ...
    `;
    await pool.query(logQuery, [warehouseId, item_id, quantity_change, reason || 'Manual inventory adjustment']);
...
router.get('/orders', async (req: Request, res: Response) => {
  try {
    ...
    const result = await pool.query(query, [warehouseId, status, Number(limit)]);
...
// POST /api/warehouse/orders/:id/assign - assign order to this warehouse
router.post('/orders/:id/assign', requirePermission('WAREHOUSE_ASSIGN'), async (req: Request, res: Response) => {
  try {
    const warehouseId = getUserId(req) || 'WH-000';
    const id = String(req.params.id);
    const r = await pool.query(`UPDATE orders SET assigned_warehouse=$1, updated_at=NOW() WHERE order_id=$2 RETURNING order_id, assigned_warehouse`, [warehouseId, id]);
    if (r.rowCount === 0) return res.status(404).json({ success: false, error: 'Order not found' });
...
// POST /api/warehouse/shipments - create outbound shipment for an order (products/supplies)
// Body: { order_id, carrier?, tracking_number?, destination_address? }
router.post('/shipments', requirePermission('WAREHOUSE_SHIP'), async (req: Request, res: Response) => {
  ...
  try {
    ...
    await pool.query(
      `INSERT INTO warehouse_shipments(shipment_id, warehouse_id, shipment_type, carrier, tracking_number, destination_address, status)
       VALUES ($1,$2,'outbound',$3,$4,$5,'pending')`,
      [shipmentId, warehouseId, carrier || null, tracking_number || null, destination_address || null]
    );

    // Load order items that are products/supplies
    const items = (await pool.query(
      `SELECT order_item_id, item_id, item_type, quantity
       FROM order_items WHERE order_id=$1 AND item_type IN ('product','supply')`,
      [order_id]
    )).rows;

    // Insert shipment items (defer inventory decrement to delivery stage)
    for (const it of items) {
      await pool.query(
        `INSERT INTO shipment_items(shipment_id, order_id, item_id, item_type, sku, item_name, quantity, unit_cost)
         SELECT $1, $2, ii.item_id, ii.item_type, ii.sku, ii.item_name, $3, ii.unit_cost
         FROM inventory_items ii WHERE ii.warehouse_id=$4 AND ii.item_id=$5`,
        [shipmentId, order_id, it.quantity, warehouseId, it.item_id]
      );
    }

    // Update order status
    await pool.query(`UPDATE orders SET status='shipped', updated_at=NOW(), assigned_warehouse=COALESCE(assigned_warehouse,$1) WHERE order_id=$2`, [warehouseId, order_id]);
...
// PATCH /api/warehouse/shipments/:id/deliver - mark a shipment as delivered and decrement stock
router.patch('/shipments/:id/deliver', requirePermission('WAREHOUSE_SHIP'), async (req: Request, res: Response) => {
  try {
    const id = String(req.params.id);
    await pool.query('BEGIN');
    // Load shipment header for warehouse_id and order_id
    const sh = (await pool.query(`SELECT warehouse_id, order_id, destination_address FROM warehouse_shipments WHERE shipment_id=$1`, [id])).rows[0];
    if (!sh) {
      await pool.query('ROLLBACK');
      return res.status(404).json({ success: false, error: 'Shipment not found' });
    }
    const warehouseId = sh.warehouse_id as string;
    const orderId = sh.order_id as string;
    const destAddress = sh.destination_address as string | null;
    // Load shipment items
    const items = (await pool.query(`SELECT item_id, quantity FROM shipment_items WHERE shipment_id=$1`, [id])).rows;
    for (const it of items) {
      await pool.query(
        `UPDATE inventory_items
        ...
        [Number(it.quantity || 0), warehouseId, it.item_id]
      );
      await pool.query(
        `INSERT INTO warehouse_activity_log(warehouse_id, activity_type, item_id, quantity_change, description, shipment_id)
         VALUES ($1,'ship',$2,$3,$4,$5)`,
        [warehouseId, it.item_id, -Math.abs(Number(it.quantity || 0)), `Delivered order ${orderId}`, id]
      );
    }
    await pool.query(`UPDATE warehouse_shipments SET status='delivered', actual_delivery_date=NOW(), updated_at=NOW() WHERE shipment_id=$1`, [id]);
    // Check if recurring; if yes, enqueue next pending shipment
    const ord = (await pool.query(`SELECT order_kind, recurrence_interval FROM orders WHERE order_id=$1`, [orderId])).rows[0];
    if (ord && ord.order_kind === 'recurring') {
      ...
      await pool.query(`UPDATE orders SET status='delivered', completion_date=NOW(), updated_at=NOW() WHERE order_id=$1`, [orderId]);
    }
    await pool.query('COMMIT');
...
// PATCH /api/warehouse/shipments/:id/cancel - cancel a shipment (no stock changes)
router.patch('/shipments/:id/cancel', requirePermission('WAREHOUSE_SHIP'), async (req: Request, res: Response) => {
  try {
    const id = String(req.params.id);
    await pool.query(`UPDATE warehouse_shipments SET status='cancelled', updated_at=NOW() WHERE shipment_id=$1`, [id]);
    // Optional: log cancellation
    await pool.query(`INSERT INTO warehouse_activity_log(warehouse_id, activity_type, description, shipment_id)
