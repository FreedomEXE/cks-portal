...
import { userCreationSchema, userUpdateSchema, roleAssignmentSchema } from '../validators/users.schema';
  ...
  async (req, res) => {
    try {
      ...
      const currentUser = await pool.query(
        'SELECT * FROM system_users WHERE user_id = $1 AND archived = false',
        [userId]
      );
      ...
      for (const field of allowedFields) {
        if (updateData[field] !== undefined) {
          ...
          updateValues.push(
            ['profile_data', 'preferences'].includes(field)
  ...
  async (req, res) => {
    try {
      ...
      const userCheck = await pool.query(
        'SELECT user_id FROM system_users WHERE user_id = $1 AND archived = false',
        [userId]
      );
      ...
      await pool.query(
        'UPDATE system_users SET role_code = $1, org_id = $2, updated_by = $3 WHERE user_id = $4',
        [role_code, org_id || null, adminId, userId]
      );
...
router.post('/:userId/reset-password',
  requireCaps('users:reset_password'),
  async (req, res) => {
    try {
      const { userId } = req.params;
