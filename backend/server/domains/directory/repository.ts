...
import { DirectoryEntityType, DirectoryItemBase, DirectoryQuery } from './types';

export async function listDirectory(userId: string, scope: 'global'|'ecosystem', roleCode: string, q: DirectoryQuery): Promise<DirectoryItemBase[]> {
  const values: any[] = [];
  ...
  let rows = res.rows as DirectoryItemBase[];
