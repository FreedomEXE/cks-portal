 ...
 * Orders.tsx
 * 
 * Description: Customer order management with service and supply order tracking
 * Function: View and manage both service orders and supply orders for customers
 * Importance: Critical - Dual order lifecycle management for customers
...
// Mock data for service orders (flow: Customer → Contractor → Manager)
...
// Mock data for supply orders (simplified flow: Customer → Contractor → Warehouse)
const mockSupplyOrders = {
  status: [
    {
      orderId: 'CUS001-ORD-SUP001',
      orderType: 'supply',
      itemName: 'Office Equipment Supplies',
      centerName: 'Downtown Office Complex',
      centerId: 'CEN001',
      ...
      {orders.map((order: any) => (
        <div key={order.orderId} className="ui-card" style={{ padding: 12, marginBottom: 8, border: '1px solid #e5e7eb' }}>
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            {order.customerStatus === 'pending' ? (
              <>
                <button disabled={actionLoading === order.orderId} onClick={() => handleOrderAction(order.orderId, 'Approve')}
                  style={{ padding: '4px 8px', borderRadius: 4, border: 'none', background: '#eab308', color: 'white', fontSize: 11, fontWeight: 600, cursor: 'pointer' }}>
            ...
            ) : order.createdBy === 'customer' && order.contractorStatus === 'pending' ? (
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
        ...
        <div style={{ border: '1px solid #e5e7eb', borderRadius: 8, padding: 12 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
            <h3 style={{ fontSize: 16, fontWeight: 600, margin: 0, color: '#eab308' }}>Service Orders</h3>
            <input
              type="text"
            ...
            {['in_progress', 'archive'].map(bucket => (
              <button
                ...
                style={{
                  ...
                  background: serviceActiveTab === bucket ? '#eab308' : 'white',
