...
/**
 * Home.tsx (Customer Hub)
 * Template shared by all Linked Customer User ID's
 * 
 * Description: Landing page for customer users with navigation dashboard
 * Function: Displays welcome message, navigation cards, and news preview
 * Importance: High - Primary navigation interface for customer users
 ...
 *        Navigation cards link to various customer features.
 */
  ...
  return (
    <Page title="Customer Hub" right={<LogoutButton navigate={navigate} />}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(180px,1fr))', gap: 12, marginTop: 16 }}>
        ...
        <NavCard to={`${base}/centers`} title="Centers" subtitle="Our locations" />
        <NavCard to={`${base}/services`} title="Services" subtitle="Service agreements" />
        <NavCard to={`${base}/invoices`} title="Invoices" subtitle="Billing & payments" />
        <NavCard to={`${base}/reports`} title="Reports" subtitle="Service reports" />
        <NavCard to={`${base}/requests`} title="Requests" subtitle="Service requests" />
