...
type NodeType = 'customer' | 'center' | 'crew';

type EcosystemNode = {
  id: string;
  name: string;
  type: NodeType;
  stats?: { centers?: number; crew?: number };
  children?: EcosystemNode[];
};

export default function CustomerEcosystemView({ code }: { code: string }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const res = await fetch(`${baseApi}/customer/ecosystem?code=${encodeURIComponent(code)}`, { credentials: 'include' });
