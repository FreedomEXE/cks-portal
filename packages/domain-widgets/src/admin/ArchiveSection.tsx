import { ActionModal, Button, DataTable, NavigationTab, PageWrapper, TabContainer } from '@cks/ui';
import { useEffect, useMemo, useState } from 'react';
import { buildOrderActions } from '../shared/utils/buildOrderActions';
...
export default function ArchiveSection({
  ...
  return (
    <PageWrapper title="Archive" showHeader={false}>
      ...
      {/* Archive Action Modal - using centralized ActionModal component with shared action builder */}
      <ActionModal
        ...
        title={selectedEntity ? `Archived ${selectedEntity.entityType.charAt(0).toUpperCase() + selectedEntity.entityType.slice(1)}` : undefined}
        archiveMetadata={selectedEntity ? {
          archivedBy: selectedEntity.archivedBy,
          archivedAt: selectedEntity.archivedAt,
          reason: selectedEntity.archiveReason,
          scheduledDeletion: selectedEntity.deletionScheduled
        } : undefined}
        actions={selectedEntity && selectedEntity.entityType === 'order' ? buildOrderActions({
          order: {
            orderId: selectedEntity.id,
            orderType: (selectedEntity as any).orderType,
          },
          state: 'archived',
          role: 'admin',
          callbacks: {
          ...
          // Non-order entities still use basic actions (could be refactored later for other entity types)
          {
            label: 'Restore Data',
