...
test.describe('Center Hub - Order Modal', () => {
  test('should open order modal from activity feed', async ({ page }) => {
    // Login as center
    await loginAs(page, 'center');
    ...
    await page.screenshot({
      path: 'test-results/center-order-modal.png',
      fullPage: true
    });
    ...
    console.log(`✅ Center order modal test complete (${openTime}ms)`);
  });
});
