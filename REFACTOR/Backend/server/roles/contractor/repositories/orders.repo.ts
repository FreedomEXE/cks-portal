...
export async function getOrdersForContractor(contractorId: string, filters: any = {}): Promise<any[]> {
...
export async function getOrderById(contractorId: string, orderId: string): Promise<any> {
  const sql = `
    SELECT 
      o.*,
      c.company_name as customer_name,
  ...
  const result = await query(sql, [contractorId, orderId]);
  return result[0] || null;
}

// Update order status
export async function updateOrder(contractorId: string, orderId: string, updateData: any): Promise<any> {
...
export async function getOrderCountsByStatus(contractorId: string): Promise<Record<string, number>> {
  const sql = `
    SELECT 
      o.status,
      COUNT(*) as count
...
export async function getAvailableJobs(contractorId: string, filters: any = {}): Promise<any[]> {
