...
export default function ContractorHub({ initialTab = 'dashboard' }: ContractorHubProps) {
  ...
  const ecosystemData = useMemo<TreeNode>(() => ({
    user: {
      id: normalizedCode ?? 'CONTRACTOR',
      role: 'Contractor',
      name: profile?.name ?? normalizedCode ?? 'Contractor',
    },
    children: [],
  }), [normalizedCode, profile]);

  const { myServicesData, serviceHistoryData } = useMemo(() => {
    const my: Array<{ serviceId: string; serviceName: string; centerId: string; type: string; status: string; startDate: string }>
      = [];
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: '#f8fafc' }}>
      <MyHubSection
        ...
        userId={normalizedCode ?? 'CONTRACTOR'}
        role="contractor"
      />
