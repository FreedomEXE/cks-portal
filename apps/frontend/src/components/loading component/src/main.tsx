...
<svg version="1.1" viewBox="0 0 1900 1848" width="475" height="462" xmlns="http://www.w3.org/2000/svg">
  <path transform="translate(0)" d="m0 0h1900v1848h-1900z" fill="#FEFEFE"/>
...
function App() {
  const [raw, setRaw] = useState(DEFAULT_SVG);
  const parsed = parseSvg(raw);

  return (
    <div className="wrap">
      <div className="card">
        <h2>Logo Loader Demo</h2>
        <p>Paste your SVG (the parser will ignore full-canvas backgrounds).</p>
        <textarea value={raw} onChange={(e)=>setRaw(e.target.value)} />
        <div className="row">
          <label>Color<input type="color" defaultValue="#111827" id="color" /></label>
          <label>Stroke (px)<input type="number" defaultValue={18} id="stroke" /></label>
          <label>Duration (s)<input type="number" step={0.1} defaultValue={1.2} id="dur" /></label>
          <label>Delay (s)<input type="number" step={0.05} defaultValue={0.25} id="delay" /></label>
          <label>Size (px)<input type="number" defaultValue={220} id="size" /></label>
        </div>
        <div className="preview">
          {parsed.paths.length ? (
            <LogoLoader
              ...
              color={(document.getElementById('color') as HTMLInputElement)?.value || '#111827'}
              size={Number((document.getElementById('size') as HTMLInputElement)?.value || 220)}
              thickness={Number((document.getElementById('stroke') as HTMLInputElement)?.value || 18)}
              duration={Number((document.getElementById('dur') as HTMLInputElement)?.value || 1.2)}
              delay={Number((document.getElementById('delay') as HTMLInputElement)?.value || 0.25)}
              stagger
            />
          ) : (
            <div>Paste a valid SVG…</div>
          )}
        </div>
      </div>
    </div>
