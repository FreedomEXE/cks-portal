...
export interface ServiceProceduresTabProps {
  serviceId: string;
  files?: Array<{ name: string; size?: number; type?: string; content?: string }>;
}

export default function ServiceProceduresTab({ serviceId, files: initialFiles = [] }: ServiceProceduresTabProps) {
  ...
  const onSelect = async (ev: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (const file of picked) {
      if (file.size <= 1024 * 1024) { // <=1MB store inline
  ...
  const onUpload = async () => {
    ...
    try {
      setUploading(true);
      await updateServiceMetadataAPI(serviceId, { procedures: files });
  ...
  const removeAt = async (idx: number) => {
    ...
    setFiles(next);
    await updateServiceMetadataAPI(serviceId, { procedures: next });
    mutate((key: any) => typeof key === 'string' && (key.includes('/services') || key.includes(serviceId)));
  };

  return (
    <div style={{ padding: 16 }}>
      <h3 style={{ margin: '0 0 8px 0', fontSize: 16, fontWeight: 600 }}>Procedures</h3>
      <p style={{ color: '#6b7280' }}>Attach procedure files (CSV/XLSX/PDF). Files up to 1MB are stored inline for MVP.</p>
