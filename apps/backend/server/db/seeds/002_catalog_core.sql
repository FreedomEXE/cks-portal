...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM catalog_services LIMIT 1) THEN
    ...
    ('SRV-001', 'Daily Maintaining Cleaning', 'Professional daily maintaining cleaning for daily areas with detail-oriented finishing and safety checks.', NULL, ARRAY['daily','interior','maintaining','cleaning'], 'daily', 'service', 120.00, 'USD', 120, 'flex', 2, jsonb_build_object('crew_required', 2), jsonb_build_object('category', 'daily'), TRUE),
