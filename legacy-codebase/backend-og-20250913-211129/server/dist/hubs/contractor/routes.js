...
router.get('/profile', async (req, res) => {
    try {
        ...
        if (!code)
            return res.status(400).json({ success: false, error: 'code required', error_code: 'invalid_request' });
        const q = `
      SELECT 
        contractor_id,
        company_name,
      ...
      LIMIT 1
    `;
        const r = await pool_1.default.query(q, [code]);
        if (r.rowCount === 0) {
        ...
        if (numCustomers === 0) {
            try {
                const c = await pool_1.default.query(`SELECT COUNT(*)::int AS c FROM customers WHERE UPPER(contractor_id)=UPPER($1)`, [row.contractor_id]);
        ...
        try {
            const fav = await pool_1.default.query(`SELECT s.service_name
         ...
         LIMIT 3`, [row.contractor_id]);
            const names = fav.rows.map((r) => r.service_name).filter(Boolean);
            if (names.length)
                servicesSpecialized = names.join(', ');
        }
        catch { }
        let accountManager = null;
        if (row.cks_manager) {
            try {
                const m = await pool_1.default.query(`SELECT manager_id, manager_name, email, phone, territory, assigned_center
           FROM managers
           WHERE UPPER(manager_id) = UPPER($1)
           LIMIT 1`, [row.cks_manager]);
                if (m.rowCount > 0)
...
router.get('/centers', async (req, res) => {
    try {
        ...
        if (!code)
            return res.status(400).json({ success: false, error: 'code required', error_code: 'invalid_request' });
        const rows = (await pool_1.default.query(`SELECT center_id AS id, center_name AS name FROM centers WHERE UPPER(contractor_id)=UPPER($1) ORDER BY center_name`, [code])).rows;
...
router.get('/my-services', async (req, res) => {
    try {
        ...
        if (!code)
            return res.status(400).json({ success: false, error: 'code required', error_code: 'invalid_request' });
        const selected = await pool_1.default.query(`SELECT cs.service_id, cs.is_favorite, s.service_name, s.category, s.description
       FROM contractor_services cs
       JOIN services s ON s.service_id = cs.service_id
       WHERE UPPER(cs.contractor_id) = UPPER($1)
       ORDER BY s.service_name`, [code]);
        const catalog = await pool_1.default.query(`SELECT service_id, service_name, category, description
       FROM services
       WHERE status = 'active'
...
router.post('/my-services', async (req, res) => {
    try {
        ...
        await pool_1.default.query(`DELETE FROM contractor_services WHERE UPPER(contractor_id)=UPPER($1) AND service_id <> ALL($2)`, [code, services.length ? services : ['#none#']]);
        for (const sid of services) {
            await pool_1.default.query(`INSERT INTO contractor_services(contractor_id, service_id, is_favorite)
         VALUES ($1,$2,FALSE)
         ON CONFLICT (contractor_id, service_id) DO NOTHING`, [code, sid]);
        }
...
router.post('/my-services/add', async (req, res) => {
    try {
        const code = String(req.body.code || '').trim() || getUserId(req);
        ...
        if (!code)
            return res.status(400).json({ success: false, error: 'code required' });
        if (!serviceId)
            return res.status(400).json({ success: false, error: 'service_id required' });
        await pool_1.default.query(`INSERT INTO contractor_services(contractor_id, service_id, is_favorite)
       VALUES ($1,$2,FALSE)
       ON CONFLICT (contractor_id, service_id) DO NOTHING`, [code, serviceId]);
        return res.json({ success: true });
    }
    catch (error) {
...
router.patch('/my-services/favorites', async (req, res) => {
    try {
        const code = String(req.body.code || '').trim() || getUserId(req);
        ...
        if (favorites.length > 3)
            return res.status(400).json({ success: false, error: 'Max 3 favorites' });
        await pool_1.default.query('BEGIN');
        for (const sid of favorites) {
            await pool_1.default.query(`INSERT INTO contractor_services(contractor_id, service_id, is_favorite)
         VALUES ($1,$2,TRUE)
         ON CONFLICT (contractor_id, service_id) DO UPDATE SET is_favorite = TRUE, updated_at = NOW()`, [code, sid]);
        }
        await pool_1.default.query(`UPDATE contractor_services
       SET is_favorite = FALSE, updated_at = NOW()
       WHERE UPPER(contractor_id) = UPPER($1) AND (ARRAY[$2]::varchar[] IS NULL OR service_id <> ALL($2))`, [code, favorites]);
        await pool_1.default.query('COMMIT');
        return res.json({ success: true });
    }
    catch (error) {
    ...
    `;
        ...
        try {
            const [p, a, r] = await Promise.all([
                pool_1.default.query(`SELECT COUNT(*)::int AS c FROM orders o WHERE o.status = ANY($1)`, [pendingStatuses]),
                pool_1.default.query(`SELECT COUNT(*)::int AS c FROM orders o WHERE o.status = ANY($1)`, [approvedStatuses]),
...
const ApproveDenySchema = zod_1.z.object({ note: zod_1.z.string().max(2000).optional() });
router.post('/requests/:id/approve', (0, rbac_1.requirePermission)('CONTRACTOR_APPROVE'), async (req, res) => {
    const id = String(req.params.id);
    const parsed = ApproveDenySchema.safeParse(req.body || {});
    const note = parsed.success ? (parsed.data.note || '') : '';
    try {
        await pool_1.default.query('BEGIN');
        await pool_1.default.query(`UPDATE orders SET status='contractor_approved', updated_at=NOW() WHERE order_id=$1`, [id]);
        await pool_1.default.query(`INSERT INTO approvals(order_id, approver_type, status, note, decided_at) VALUES ($1,'contractor','approved',$2,NOW())`, [id, note || null]);
        await pool_1.default.query('COMMIT');
...
router.post('/requests/:id/deny', (0, rbac_1.requirePermission)('CONTRACTOR_APPROVE'), async (req, res) => {
    const id = String(req.params.id);
    const parsed = ApproveDenySchema.safeParse(req.body || {});
    const note = parsed.success ? (parsed.data.note || '') : '';
    try {
        await pool_1.default.query('BEGIN');
        await pool_1.default.query(`UPDATE orders SET status='contractor_denied', updated_at=NOW() WHERE order_id=$1`, [id]);
        await pool_1.default.query(`INSERT INTO approvals(order_id, approver_type, status, note, decided_at) VALUES ($1,'contractor','denied',$2,NOW())`, [id, note || null]);
        await pool_1.default.query('COMMIT');
