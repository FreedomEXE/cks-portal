...
const CreateCustomerRequestSchema = zod_1.z.object({
...
router.post('/requests', (0, rbac_1.requirePermission)('CUSTOMER_CREATE_REQUEST'), async (req, res) => {
    try {
        ...
        try {
            ...
            await pool_1.default.query(`INSERT INTO orders(order_id, customer_id, center_id, status, notes) VALUES ($1,$2,$3,$4,$5)`, [orderId, customer_id, center_id || null, status, notes || null]);
    ...
    try {
        ...
        try {
            const [p, a, r] = await Promise.all([
                pool_1.default.query(`SELECT COUNT(*)::int AS c FROM orders o WHERE UPPER(o.customer_id)=UPPER($1) AND o.status = ANY($2)`, [code, pendingStatuses]),
                pool_1.default.query(`SELECT COUNT(*)::int AS c FROM orders o WHERE UPPER(o.customer_id)=UPPER($1) AND o.status = ANY($2)`, [code, approvedStatuses]),
                pool_1.default.query(`SELECT COUNT(*)::int AS c FROM orders o WHERE UPPER(o.customer_id)=UPPER($1) AND o.status = ANY($2)`, [code, archiveStatuses]),
            ]);
            totals = {
